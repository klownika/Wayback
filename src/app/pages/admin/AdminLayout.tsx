  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    ...
    >
      ...
      <div className="px-6 pt-7 pb-6" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <span
          style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.22em', color: '#c70fff', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}
        >
          WAYBACK
        </span>
        <span
      ...
      <nav className="flex-1 py-4 px-3 overflow-y-auto">
        {NAV.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            ...
            style={({ isActive }) => ({
              borderRadius: 6,
              background: isActive ? 'rgba(199,15,255,0.18)' : 'transparent',
              color: isActive ? '#c70fff' : 'rgba(255,255,255,0.5)',
            })}
          >
            {({ isActive }) => (
              <>
                <Icon style={{ width: 15, height: 15, flexShrink: 0, color: isActive ? '#c70fff' : 'rgba(255,255,255,0.35)' }} />
                <span style={{ fontSize: 13, fontWeight: isActive ? 600 : 400 }}>{label}</span>
                {isActive && (
                  <span
                    className="ml-auto"
                    style={{ width: 4, height: 4, borderRadius: '50%', background: '#c70fff', flexShrink: 0 }}
                  />
                )}
              </>
            )}
      ...
      <div className="px-4 pb-6 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        {user && (
          <div className="flex items-center gap-3 mb-4 px-2">
            <div
              ...
              style={{ width: 32, height: 32, background: '#c70fff', borderRadius: '50%' }}
            >
        ...
        <button
          ...
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = '#c70fff'; (e.currentTarget as HTMLButtonElement).style.background = 'rgba(199,15,255,0.08)'; }}
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#f8f7f5' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        >
          ...
          <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: '0.2em', color: '#c70fff' }}>WAYBACK</span>
